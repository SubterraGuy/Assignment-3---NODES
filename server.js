...
app.get("/api/painting/:id", (req, res) => {
    const result = paintings.find(p => p.paintingID === Number(req.params.id));
...
app.get("/api/painting/gallery/:id", (req, res) => {
    const result = paintings.filter(p => p.gallery.galleryID === Number(req.params.id));
...
app.get("/api/painting/artist/:id", (req, res) => {
    const result = paintings.filter(p => p.artist.artistID === Number(req.params.id));
    if (!result.length) return notFound(res);
    res.json(result);
});
